...
class Units201To250Tests(unittest.TestCase):
    ...
    def test_highest_unit_is_at_least_250(self) -> None:
        self.assertGreaterEqual(PHASE5_HIGHEST_UNIT, 250)
        status = json.loads(workshop_status())
        self.assertGreaterEqual(status["highest_unit"], 250)
        self.assertFalse(status["measured"])
        self.assertNotIn("claim_level", status)
        stop = json.loads(workshop_stop_record())
        self.assertGreaterEqual(stop["highest_unit"], 250)
